...
public class Grid : MonoBehaviour
{
    ...
    {
        List<Vector3> validCoords = new List<Vector3>(); // Store the coordinates of sand cells

        // Loop through the grid to find sand cells
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                Node cell = grid[x, y];
                TerrainType type = terrainGrid[x, y];  // Assuming terrainGrid is accessible and contains the terrain types

                if (type == TerrainType.Sand)
                {
                    // Add the position of the sand cell to the list
                    validCoords.Add(cell.worldPosition);
                }
            }
        }

        // Ensure there are at least two sand cells to choose from
        if (validCoords.Count >= 2)
        {
            // Select two distinct random indices from validCoords
            int index1 = Random.Range(0, validCoords.Count);
            int index2;

            do
            {
                index2 = Random.Range(0, validCoords.Count);  // Keep selecting until a different index is chosen
            } while (index1 == index2);

            // Get the coordinates of the two randomly selected sand cells
            Vector3 coord1 = validCoords[index1];
            Vector3 coord2 = validCoords[index2];

            // Instantiate the prefabs at the selected coordinates
            Instantiate(towerSpawn, coord1, Quaternion.identity);
            Instantiate(enemySpawn, coord2, Quaternion.identity);
